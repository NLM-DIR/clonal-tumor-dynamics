...
##########
# This script estimates the terms c, a, f, b, and l for the experiments with
...
# 'c_{cmin}_{cmax}_{cinc}_a_{amin}_{amax}_{ainc}_f_{fmin}_{fmax}_{finc}_b_{bmin}_{bmax}_{binc}_l_{lmin}_{lmax}_{linc}.csv'
##########


def main(config, c_min, c_max, c_inc, a_min, a_max, a_inc, f_min, f_max, f_inc, b_min, b_max, b_inc, l_min, l_max, l_inc, save_path):
    ...
    print("Precision: d=", c_precision, "a=", a_precision, "f=", f_precision, "b=", b_precision, "l=", l_precision)
    ...
    print("Number of grid elements:", len(c_list)*len(a_list)*len(f_list)*len(b_list)*len(l_list), flush=True)
    ...
    results = es.b6_all(gkm_pairs, c_list, a_list, f_list, b_list, l_list)
    
    print("Time:", time.time() - start_time)
    results = results[results["error"] != np.inf]
    results = results.astype({"id": "uint16", "g1":"float32", "g11":"float32", "m":"float32", "k":"float32", 
                              "c":"float32", "a":"float32", "f":"float32", "b":"float32", "l":"float32", 
                              "error":"float32", "winner":"uint8"})

    # Save results
    results.to_csv("{}c_{}_{}_{}_a_{}_{}_{}_f_{}_{}_{}_b_{}_{}_{}_l_{}_{}_{}.csv".format(
         save_path, c_min, c_max, c_inc, a_min, a_max, a_inc,
         f_min, f_max, f_inc, b_min, b_max, b_inc, l_min, l_max, l_inc), index=False)
    print(results)
...
if __name__ == "__main__":
    ...
    main(args.config, args.cmin, args.cmax, args.cinc, args.amin, args.amax, args.ainc, 
         args.fmin, args.fmax, args.finc, args.bmin, args.bmax, args.binc, args.lmin, args.lmax, args.linc, args.save_path)